...
import { ManagementObj, SchemeObj } from '@/types'
import dayjs from 'dayjs'
import zh_CN from 'antd/es/locale/zh_CN';
import '../../pages/index.less'
import { getMenuData } from '_@ant-design_pro-layout@6.27.2@@ant-design/pro-layout';


interface Props {
    edit: (item: ManagementObj) => void,
...
const Managements = (props: Props) => {
    ...
    const del = (item: ManagementObj) => {
        dispatch({
          type: 'Management/deladmin',
          payload: {
            id: item.id,
            }
        })
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (isEmplower: number) => (
                <>
                    <div className={isEmplower == 1 ? '' : 'role-status'}>
                        {isEmplower == 1 ? "启用" : "禁用"}
                    </div>
                </>
            )
