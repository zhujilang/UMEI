...
//   let total = useSelector((state: any) => state.Notice.total)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status: number) => (
        <>
        <div className={status==0?'status-blue':status=='下架'?'status-grey':status=='审核中'?'status-green':'status-red'}>
          {status}
        </div>
        </>
      )
  ...
  return (
    <div>
      ...
      <Table rowKey="_id" columns={columns as any} dataSource={data} className={'m-t10'}
        pagination={false} />
