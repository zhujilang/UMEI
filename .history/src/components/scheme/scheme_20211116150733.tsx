...
const Schemes = (props: Props) => {
  let arr = useSelector((state: any) => state.Scheme.SchemeInfo)
...
//   换页
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (createTime: any) => (
        <span>{dayjs(createTime).('YYYY-MM-DD HH:mm:ss')}</span>
      )
    },
    {
        title: '上架时间',
  ...
  return (
    <div>
      <Table rowKey="_id" columns={columns as any} dataSource={arr} className={'m-t10'}
        pagination={false} />
...
function dayjs(pay_time: string) {
