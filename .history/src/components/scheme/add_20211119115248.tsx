          ...
          <Form.Item>
            ...
            </div>
            {
                            schemes.map((item, index) => {
                                return <div className="m-scheme-detail-container" key={index}>
                                    {
                                        index === 0 ? "" : <div className="m-scheme-remove" onClick={() => {

                                            if (schemes.length === 1) {
                                                Modal.error({
                                                    title: '错误信息',
                                                    content: '「方案详情」不能全部删除，至少保留一项！',
                                                });
                                            } else {
                                                const result = schemes.filter((v, i) => {
                                                    return index !== i;
                                                });
                                                setSchemes(result);
                                            }
                                    ...
                                    <div className="m-scheme-detail-content">
                                        <div>标题</div>
                                        <div>
                                    ...
                                    <TextArea placeholder="在这里编辑方案描述" value={item.description} onChange={(event) => {
                                        const clone = Object.assign({}, item, {
                                            description: event.target.value
                                        });
                                        setSchemes([...schemes.slice(0, index), clone, ...schemes.slice(index + 1)]);
                                    }}></TextArea>
                                </div>
                            })
                        }
