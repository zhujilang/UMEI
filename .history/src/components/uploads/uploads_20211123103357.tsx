...
const Uploadss = (props: Props) => {
  ...
  const attr: any = {
    ...
    name: 'file',
    action: 'http://47.108.69.213:8081/admin/core/file/upload',
    headers: {
    ...
    <div>
      {
        props.listType === 'picture' ?
          ...
          :
          props.fileList && props.fileList.length <!DOCTYPE html>
          <html lang="en">
          <head>
              <meta charset="UTF-8" />
                ...
                props.fileList.map((item: any,index:number) => {
                  return (
                    <div className={'p-r'} key={index} style={{border:'1px solid #eee'}}>
                      ...
                      <Image width={80} height={80} src={item.url}></Image>
                    </div>
                  )
                })
              }</div>
      ...
      }
      {
        src && props.listType === 'picture' ? <div className={'m-t20'}>
          <Image src={src} width={props.imgW ? props.imgW : 100} height={props.imgH ? props.imgH : 35}></Image>
        </div> : null
      }
      <Modal
