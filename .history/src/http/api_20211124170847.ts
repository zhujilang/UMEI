...
export default {
  ...
  //人员管理列表
  getadminlist({pageNum,pageSize}:{pageNum:number,pageSize:number}) {
    return http.get(`/admin/list/${pageNum}/${pageSize}`)
  },
  //删除人员
  deladmin({id}:{id:number}) {
    return http.post(`/admin/delete/${id}`)
  },
  //获取时尚资讯列表
  getfashionlist({pageNum,pageSize,status,keyword}:{pageNum:number,pageSize:number,status:string,keyword:string}) {
    return http.post('/admin/baseFashion/list',{
      pageNum,pageSize,status,keyword
    })
  },
  // 获取售后列表
  getaftersalelist({pageNum,pageSize,status}:{pageNum:number,pageSize:number,status:number}) {
    return http.get(`/admin/aftermarket/decoration?pageNum=${pageNum}&pageSize=${pageSize}&status=${status}`)
  },
  //订单列表
  getlistOrders({pageNum,pageSize,orderStatus}:{pageNum:number,pageSize:number,orderStatus:number}) {
    return http.get(`/admin/aftermarket/decoration?pageNum=${pageNum}&pageSize=${pageSize}&orderStatus=${orderStatus}`)
  },
}