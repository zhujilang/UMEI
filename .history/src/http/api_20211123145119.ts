...
export default {
  ...
  //获取时尚资讯列表
  getfashionlist({pageNum,pageSize,status,keyword}:{pageNum:number,pageSize:number,status:strinnumg,keyword:string}) {
    return http.get(`/admin/baseFashion/list?pageNum=${pageNum}&pageSize=${pageSize}&status=${status}&keyword=${keyword}`)
  },
}