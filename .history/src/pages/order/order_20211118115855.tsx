...
const Order = () => {
    ...
    // 获取订单数据
    const getData = (pageNum: number, pageSize: number, orderStatus: string) => {
        dispatch(
            {
    ...
    useEffect(() => {
        getData(pageNum, pageSize, orderStatus)
    }, [])
    //切换页码
    const onPageChange = (pageNum: number, pageSize: number,orderStatus:any) => {
        setPageNum(pageNum)
        setPageSize(pageSize)
        pageNum > 0 && getData(pageNum, pageSize, orderStatus)
    }
    return (
        <div className={'order'}>
            <div className={'order-all'}>
                ...
                <div>
                <Orders  onPageChange={onPageChange} pageSize={pageSize} pageNum={pageNum} orderStatus={orderStatus} ></Orders>
                </div>
            </div>
...
function orderStatus(pageNum: number, pageSize: number, orderStatus: any) {
