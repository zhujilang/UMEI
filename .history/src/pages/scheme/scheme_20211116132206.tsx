...
const Scheme = () => {
  ...
  let [pageNum, setCurrent] = useState(1)
  ...
  // 获取整屋数据
  const getData = (pageNum: number, pSize: number, queryVal: string) => {
    dispatch(
      {
  ...
  const onPageChange = (page: number, pageSize: number) => {
    ...
    page > 0 && getData(page, pageSize, status)
  }
    return (
        <div className={'scheme'}>
            <div className={'scheme-all'}>
                ...
                <div>
                <Schemes edit={edit} onPageChange={onPageChange} status={status} pageSize={pageSize} current={current}></Schemes>
                </div>
            </div>
