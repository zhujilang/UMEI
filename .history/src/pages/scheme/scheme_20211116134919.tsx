...
const Scheme = () => {
  ...
  // 获取整屋数据
  const getData = (pageNum: number, pageSize: number, statusVal: string) => {
    dispatch(
      {
  ...
  const onPageChange = (page: number, pageSize: number) => {
    ...
    page > 0 && getData(pageNum, pageSize, status)
  }
    return (
        <div className={'scheme'}>
            <div className={'scheme-all'}>
                ...
                <div>
                <Schemes edit={edit} onPageChange={onPageChange} status={status} pageSize={pageSize} pageNum={pageNum} key={undefined} goodsTitle={''} purchaseInfo={undefined} categoryId={undefined} price={undefined} createTime={undefined}></Schemes>
                </div>
            </div>
